...
function renderFavRow(){
  ...
  for(let i=0;i<9;i++){
    ...
    const onDown=(e)=>{ e.preventDefault(); long=false; clear();
      vibrateTimer=setTimeout(()=>{ vibrate(50); },1000);
...
(function(){
  ...
  st.textContent = '.lm.meta{font-family:ui-monospace,Menlo,Consolas,monospace;font-size:11px;opacity:.8;margin-left:6px;}.lm-toggle{font-size:12px;opacity:.85}';
  document.head.appendChild(st);

  // Create toggle and place near "Master Brightness"
  function findByText(sel, rx){
    var els = document.querySelectorAll(sel);
    for (var i=0;i<els.length;i++){
      var t = (els[i].textContent||'').trim();
      if (rx.test(t)) return els[i];
    }
    return null;
  }

  var host = findByText('label, h2, h3, div, span', /master\s*brightness/i) || document.body;
  var lab = document.createElement('label');
  lab.className = 'lm-toggle';
  lab.style.display = 'inline-block';
  lab.style.marginLeft = '8px';
  lab.innerHTML = '<input id="lmToggle" type="checkbox"> Show live metrics';
  host.appendChild(lab);

  var elToggle = document.getElementById('lmToggle');
  if (!elToggle) return;

  // Attach meta spans after each Speed/Intensity slider
  var metas = [];
  var SPEED_RX = /speed/i, INT_RX = /intensity/i;

  function makeSpan(){
    var sp = document.createElement('span');
    sp.className = 'lm meta';
    sp.hidden = !elToggle.checked;
    return sp;
  }
  ...
  labels.forEach(function(lab){
    ...
    if (!txt) return;
    var container = lab.parentElement || lab;
    var range = container.querySelector('input[type="range"]');
    if (!range) return;
    if (SPEED_RX.test(txt)){
      var sp = makeSpan();
      range.insertAdjacentElement('afterend', sp);
      metas.push({range:range, span:sp, type:'speed'});
    } else if (INT_RX.test(txt)){
      var sp2 = makeSpan();
      range.insertAdjacentElement('afterend', sp2);
      metas.push({range:range, span:sp2, type:'intensity'});
    }
  });

  function clamp01(x){ return x<0?0:(x>1?1:x); }
  function round(n,d){ var f=Math.pow(10,d||2); return Math.round(n*f)/f; }
  function periodFromS(s){
    var lnHi = Math.log(3000), lnLo = Math.log(40);
    var lnP  = lnHi + (lnLo - lnHi) * s;
    return Math.round(Math.exp(lnP));
  }

  function update(){
    metas.forEach(function(m){
      var min = Number(m.range.min||0);
      var max = Number(m.range.max||100);
      var val = Number(m.range.value||0);
      if (m.type === 'speed'){
        var s = clamp01((val - min) / (max - min));
        var p = periodFromS(s);
        var hz = p>0 ? (1000/p) : 0;
        m.span.textContent = 's='+round(s,2)+'  period='+p+' ms  Hz='+round(hz,1);
      } else {
        m.span.textContent = 'i='+val;
      }
    });
  }

  elToggle.addEventListener('change', function(){
    var on = elToggle.checked;
    metas.forEach(function(m){ m.span.hidden = !on; });
    try{ localStorage.setItem('lm_on', on?'1':'0'); }catch(e){}
    if (on) update();
  });

  try{
    elToggle.checked = localStorage.getItem('lm_on') === '1';
  }catch(e){}
  elToggle.dispatchEvent(new Event('change'));

  document.addEventListener('input', function(ev){
    if (ev && ev.target && ev.target.type === 'range') update();
  });
